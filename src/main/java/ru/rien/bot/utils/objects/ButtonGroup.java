    ...
    public static class Button {
        ...
        public void addReaction(Message message) {
            if (!(message.getChannel().getType() == ChannelType.TEXT && message.getGuild().getSelfMember()
                ...
                message.getChannel().sendMessage("У меня нет прав на просмотр истории чата").queue();
                return;
            }
            ...
            if (unicode != null)
                ...
                try {
                    message.addReaction(ModuleDsBot.getInstance().getJda().getEmoteById(emoteId)).queue();
