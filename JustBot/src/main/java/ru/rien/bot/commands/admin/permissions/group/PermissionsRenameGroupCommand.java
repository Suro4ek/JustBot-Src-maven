...
public class PermissionsRenameGroupCommand implements SubCommand {
    @Override
    public SubcommandData getSubCommands() {
        return new SubcommandData("rename", "переименовать группу");
    }
            ...
            if (guild != null) {
                ...
                if (member != null) {
                    if (member.getPermissions().contains(net.dv8tion.jda.api.Permission.ADMINISTRATOR)) {
                        ...
                        String new_group_name = optionMappingList.get(2).getAsString();
                        if (guildWrapper.getPermissions().renameGroup(group, new_group_name)) {
                            MessageUtils.sendMessage("Группа переименована", event.getEvent().deferReply(true));
                        } else {
                            MessageUtils.sendWarningMessage("Не удалось переименовать (возможно уже такое имя есть)", event.getEvent().deferReply(true));
                        }
                    } else {
