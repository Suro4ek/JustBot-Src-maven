...
public class PermissionsMoveGroupCommand implements SubCommand {
    @Override
    public SubcommandData getSubCommands() {
        return new SubcommandData("move", "сдвинуть группу");
    }
            ...
            if (guild != null) {
                ...
                if (member != null) {
                    if (member.getPermissions().contains(net.dv8tion.jda.api.Permission.ADMINISTRATOR)) {
                        ...
                        int pos = (int) optionMappingList.get(2).getAsLong();
                        if (pos < 1 || pos >= guildWrapper.getPermissions().getGroups().size()) {
                            MessageUtils.sendWarningMessage("Неверная позиция : " + pos, event.getEvent().deferReply(true), user);
                        } else {
                            guildWrapper.getPermissions().moveGroup(group, pos - 1);
                            MessageUtils.sendSuccessMessage("Группа `" + group_name + "` сдвинута на позицию " + pos, event.getEvent().deferReply(true), user);
                        }
                    } else {
    ...
    public OptionData[] parameters() {
        return new OptionData[]{new OptionData(OptionType.STRING, "guild_id", "ваш id сервера", true),
                ...
                new OptionData(OptionType.INTEGER, "position", "на сколько сдвинуть", true)
        };
    }
}
