package ru.rien.bot.commands.admin.permissions.user;
...
public class PermissionsGroupAddUser implements SubCommand {
    @Override
    public SubcommandData getSubCommands() {
        return new SubcommandData("groupadd", "добавить группу пользователя");
    }
            ...
            if(guild != null){
                ...
                if(member != null) {
                    if (member.getPermissions().contains(net.dv8tion.jda.api.Permission.ADMINISTRATOR)) {
                        ...
                        if (group == null) {
                            MessageUtils.sendErrorMessage("Такой группы нет. Создайте её `/permissions group [guild_id] " + group_name + " create`", replyAction);
                            return;
                        }
