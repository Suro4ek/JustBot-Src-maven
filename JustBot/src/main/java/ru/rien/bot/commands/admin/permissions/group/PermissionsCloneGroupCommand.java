...
public class PermissionsCloneGroupCommand implements SubCommand {
    @Override
    public SubcommandData getSubCommands() {
        return new SubcommandData("clone", "клонировать группу");
    }
            ...
            if (guild != null) {
                ...
                if (member != null) {
                    if (member.getPermissions().contains(net.dv8tion.jda.api.Permission.ADMINISTRATOR)) {
                        ...
                        if (guildWrapper.getPermissions().cloneGroup(group, new_group_name)) {
                            MessageUtils.sendMessage("Клон группы был создан", event.getEvent().deferReply(true));
                        } else {
                            MessageUtils.sendWarningMessage("Не удалось создать клона (возможно он уже есть)", event.getEvent().deferReply(true));
                        }
                    } else {
