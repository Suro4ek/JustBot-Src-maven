...
public class DuckCommand implements Command {
    ...
    public void execute(CommandEvent event) {
        try {
            HttpResponse<JsonNode> response = Unirest.get("https://random-d.uk/api/v1/random?type=gif,png").asJson();
    ...
    public String getDescription(GuildWrapper guildWrapper) {
        return "Случайная утка";
    }

    @Override
    public String getUsage(GuildWrapper guildWrapper) {
        return "{%}duck - Случайная утка";
    }
    ...
    public Permission getPermission() {
        return Permission.DUCK_COMMAND;
    }
}