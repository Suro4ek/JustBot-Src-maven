...
import okhttp3.*;
import ru.rien.bot.api.zip.GzipRequestInterceptor;
...
public class WebUtils {
    ...
    private static final DataInterceptor dataInterceptor = new DataInterceptor(DataInterceptor.RequestSender.JUSTBOT);
    ...
    private static final Callback defaultCallback = new Callback() {
        ...
        public void onFailure(Call call, IOException e) {
            ModuleDsBot.getInstance().getLogger().error("Error for " + call.request().method() + " request to " + call.request().url(), e);
        }
        ...
        public void onResponse(Call call, Response response) {
            response.close();
            ModuleDsBot.getInstance().getLogger().debug("Response for " + call.request().method() + " request to " + call.request().url());
        }
    };
    ...
    public static Response request(Request.Builder builder) throws IOException {
        return ModuleDsBot.getClient().newCall(builder.build()).execute();
    }
