    ...
    public boolean addVote(Vote vote, User user) {
        ...
        if (!users.contains(user.getIdLong())) {
            ...
            if (allowedUsers.size() == totalVotes()) {
                VoteUtil.finishNow(id, GuildUtils.getGuildById(voteMessage.getId()));
                return true;
            }
            ...
            if (voteMessage != null) {
                ...
                votesEmbed.addField("Да", String.valueOf(yesVotes), true);
                votesEmbed.addField("Нет", String.valueOf(noVotes), true);
